<?php
	...
	if(isset($_POST['op']) && $_POST['op'] == "update"){
		...
		if(empty($errors)){
			...
			// update record
			$stmt_update = $mysqli->prepare("UPDATE " . $table . " SET title = ?, percentage = ?, active = ? WHERE " . $pk . " = " . $entity->getDBValue($pk) . " AND language_id = " . $language_id) or die('<h3>Preparing statement...</h3>' . $mysqli->error);
...
<html>
    ...
    <body>
        ...
        <div id="data_container">
            ...
            <form class="form_model" name="update_product_size_form" method="post" action="<?= $_SERVER['REQUEST_URI']; ?>">
                <div class="form_pane">
                    <table>
                        <tr>
                            <th>Nome *</th>
