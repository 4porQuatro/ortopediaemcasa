<?php
	...
	if(isset($_POST['op']) && $_POST['op'] == "update"){
		...
		if(empty($errors)){
			...
			// update record
			$stmt_update = $mysqli->prepare("UPDATE " . $table . " SET comment = ?, item_id = ?, user_id = ?, active = ? WHERE " . $pk . " = " . $entity->getDBValue($pk) . " AND language_id = " . $language_id) or die('<h3>Preparing statement...</h3>' . $mysqli->error);
                    ...
                    <tr>
                        <td>
                        	<select name="item_id">
								...
								?>
								<option value="<?= $rec->item_id ?>"<?= $selected ?>><?= $rec->title ?></option>
								<?php
									}
								?>
                            </select>
                        ...
                        <td><input type="text" value="<?= $entity->getDBValue('created_at') ?>" disabled></td>
                    </tr>
                </table>
