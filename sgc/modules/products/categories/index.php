...
<body>
            	...
            	<tr>
                    <td style="width:33%">
                    	<select>
							...
							<?php
                                $result_filters = $mysqli->query("SELECT * FROM items_types WHERE language_id = " . $language_id . " ORDER BY priority ASC;") or die($mysqli->error);
    ...
    $(document).ready(function(){
        $('.records_pane').recordsList({
            ...
            filters_arr: ['type_id', 'active', 'highlight'],
            sort_field: 'priority',
			sortable: true
        });
    });
