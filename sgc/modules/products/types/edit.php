<?php
	...
	if(isset($_POST['op']) && $_POST['op'] == "update"){
		...
		if(empty($errors)){
			...
			// update record
			$stmt_update = $mysqli->prepare("UPDATE " . $table . " SET slug = ?, title = ?, subtitle = ?, active = ? WHERE " . $pk . " = " . $entity->getDBValue($pk) . " AND language_id = " . $language_id) or die('<h3>Preparing statement...</h3>' . $mysqli->error);
