 ...
 * @param $node_id
 */
function getTreePath($mysqli, $table, $node_id)
{
    $rs_node = $mysqli->query("SELECT * FROM " . $table . " WHERE id = " . $node_id) or die($mysqli->error);
    if($rs_node->num_rows)
    {
        $node = $rs_node->fetch_object();

        if($node->parent_id !== null)
        {
            getTreePath($mysqli, $table, $node->parent_id);
        }
