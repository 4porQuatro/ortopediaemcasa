/*
*	JQuery Docs Uploader Plugin v2.0
*	Developed by 4por4
*	Author: José Osório
*/
(function($){
	$.fn.docsUploader = function(options){
		var path = $.fn.docsUploader.findIncludePath();

		// append CSS
		if(!$('#docs_uploader_styles').length){
			$('head').append('<link id="docs_uploader_styles" rel="stylesheet" type="text/css" href="' + path + 'assets/css/layout.css" media="all">');
		}
		...
		return this.each(function(){
			...
			// initialize plugin
			var init = function(){
				opts = $.extend($.fn.docsUploader.defaults, options);
			...
			var setElements = function(){
				...
				$this.wrap('<div class="doc_placeholder"></div>');
				...
				$placeholder.append('<div class="button">clique aqui para selecionar um documento</div>');
				$placeholder.after('<ul class="sortable_docs"></ul><hr class="clear">');
				$placeholder.before('<hr class="clear"><table><tr><td><b>Notas:</b><br><b>1.</b> Os documentos podem ter o tamanho máximo de ' + mbs + ' MB.<br><b>2.</b> Os documentos devem estar num dos seguintes formatos: <i>' + opts.allowed_formats.join(', ').toUpperCase() + '</i>.<br></td></tr></table>');
				...
				// set list
				$list = $wrapper.find('.sortable_docs');
			...
			var updateElements = function(){
				$title_input = $list.find('input[name="doc_title[]"]')
				$title_input.on('keyup', updateTitle);
				$delete_btns = $list.find('.delete_doc_btn').on('click', deleteFile);
				...
				if(files){
					...
					for(var i = 0; i < files.length; i++){
						if(files[i].size > opts.max_size){
							errors += "O tamanho do documento \"" + files[i].name + "\" não é válido.\n";
						}
						...
						if(opts.allowed_formats.indexOf(file_extension) < 0){
							errors += "O formato do documento \"" + files[i].name + "\" não é válido.\n";
						}
					}
	...
	*	FIND INCLUDE PATH
	*/
	$.fn.docsUploader.findIncludePath = function(){
		var scripts = document.getElementsByTagName('script'),
			src, path;

		for(var i = 0; i < scripts.length; i++){
			var script = scripts[i];
			if(script.src.indexOf('DocsUploader') >= 0){
				src = script.src;
			}
		}
