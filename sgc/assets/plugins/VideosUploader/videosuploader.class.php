...
class VideosUploader
{
    ...
    {
        return dirname($_SERVER['DOCUMENT_ROOT']) . "/public/uploads/videos/";
    }
    ...
    public static function uploadFiles()
    {
        ...
        foreach ($files as $key => $file) {
            ...
            if (!empty($file['name'])) {
                ...
                $video_extension = strtolower($extension[sizeof($extension) - 1]);

                if ($video_size < 0 || $video_size > $max_size)
            ...
            if (empty($errors)) {
                $video_source = FileUtilities::generateFilename($folder, $file['name']);

                move_uploaded_file($file['tmp_name'], $folder . $video_source);
                if (file_exists($folder . $video_source)) {
                    array_push($return_arr['files'], array('filename' => $filename, 'source' => $video_source, 'title' => NULL));
                }
            } else {
